...
Knowledge representation is part of knowledge-based AI and deals with the abilities of knowing and reasoning.

Inspired by one of my projects, the [openArtBrowser](https://openartbrowser.org), I will use creative arts as the sample application domain for this chapter and the remainder of the book.
...
![Fig. 3.4: knowledge graph facts and schema](images/KG_Meta_Concepts.png)

A knowledge graph consists of *facts* and a *schema*. Facts are based on the schema. Facts are individuals (concept instances, e.g., Michelangelo) and concrete relationships between those individuals (e.g., Michelangelo created David; dashed lines in the figure). The schema specifies the type level (solid lines in the figure). Classes are the concept types for individuals (e.g., artist for Michelangelo). Relationship types, e.g., "created" define the kinds of relationships that can be specified between individuals. Finally, *rules* are part of a knowledge graph schema. 
...
Knowledge representation is no end in itself but a means to an end: answering relevant questions of the application domain. Therefore, knowledge graph technologies include a query engine that allow to formulate queries and retrieve results.
...
Q3 (Is Michelangelo a painter?) also expects a Boolean answer but cannot be answered by simply matching explicitly stated facts. Instead, reasoning is required to derive the answer: Michelangelo is a person and painted „Creation of Adam“; every person who has created a painting is a painter => Michelangelo is a painter. Hence, the answer is yes.

Q4 (Which painters exist?) also involves reasoning but expects a set of individuals as an answer. In case the knowledge graph only contained the facts mentioned above then the result would, indeed, be just {Michelangelo}. If, however, paintings by Raphael, Leonardo da Vinci, etc. were listed then the set would contain those artists as well.

### Open versus Closed World Assumption

Q4 (Which painters exist?) leads to an interesting question: How to ensure that the knowledge graph is complete, i.e., actually contains all painters?

In a knowledge graph for our solar system it is relatively easy to list all planets (even though the definition of what is a planet may change over time: Pluto is, today, considered a dwarf planet only). In contrast, in the Art knowledge graph it is next to impossible to list all painters that ever lived. Many of them are not known anymore. And who should decide who was a "real" painter and who just scribbled a few sketches?

Other critical questions are counting questions (How many painters exist?) and negated questions (Which painters have not created a sculpture?).  The answers to those questions may be wrong if the knowledge graph is incomplete -- even if all facts in the knowledge graph are, indeed, correct.

Therefore, the correct interpretation of knowledge graph query results depends on assumptions that we make on the completeness of the knowledge graph[^fn-uncertainty]. 
...
In RDF, every resource is identified by a *Internationalized Resource Identifier* ([IRI](https://www.w3.org/TR/rdf11-concepts)).
...
the IRI above can briefly be specified as:
...
Then, the IRI above can be specified without prefix at all: 

    :Q5592

Note that the IRI does not need to be a cryptic ID but can also be a symbolic name, e.g.
...
[Linked Data](http://linkeddata.org/) is an initiative to create, interlink, and share knowledge graphs for a wide range of application areas. Linked data is based on the W3C Semantic Web technologies introduced above. A large number of most comprehensive knowledge graphs has been created and are publicly available -- see the [Linked Open Data Cloud](https://lod-cloud.net) in Fig. 3.10.

![Fig. 3.10: Linked Open Data (LOD) Cloud](images/LOD_Cloud.png)

Prominent linked data knowledge graphs are [WikiData](https://www.wikidata.org), [DBpedia](http://wiki.dbpedia.org/), and [YAGO](https://yago-knowledge.org/). 
...
### Example: Art Knowledge Graph

In this book, I use an Art knowledge graph in RDF as an example. The Art knowledge graph is a custom subset of Wikidata. Wikidata itself is a crowd-sourced community effort to provide the information for Wikipedia's info boxes and one of the most comprehensive publicly available knowledge graph.

Wikidata provides a crawling API for extracting custom knowledge graphs for AI applications. The art knowledge graph crawler has been developed in one of my research projects (Humm 2020). The crawler code is open source under [GitHub](https://github.com/hochschule-darmstadt/openartbrowser/blob/master/scripts/data_extraction/art_ontology_crawler.py). 

The Art knowledge graph consists of ca. 1.9 million RDF triples, representing
...
[Protégé](http://protege.stanford.edu/) is an open source knowledge graph editor.  Fig. 3.12 shows as an example, the entry of Mona Lisa in the Art knowledge graph in Protégé. 
...
Each triple poses a restriction on the query -- implicitly AND connected. We query for some entry `?p` with label `"Leonardo da Vinci"` and some `date_of_bith` entry `?d`. If such a combination can be found in the knowledge graph then it is returned as the query result.

Fig. 3.13 shows this query and its result executed in the SPARQL server [Apache Jena Fuseki](https://jena.apache.org/documentation/fuseki2/) after loading the Art knowledge graph. 

{width=75%}
![Fig. 3.13: A simple SPARQL query in Fuseki](images/SPARQL_Simple_Query.png)

X> Since the Art knowledge graph is an extract of Wikidata, you can execute a similar query at the [Wikidata query service](https://query.wikidata.org)  (See Fig. 3.14). Since Wikidata uses cryptic IDs for all properties (e.g., `wdt:P569` for `date_of_birth`, there is a query helper suggesting parts of the query to be completed.  Try it yourself! 
...
Executed on the Art knowledge graph, this query will result in 690 artists, including Édouard Manet, Claude Monet, and Jacques Rousseau. 
...
The following query lists countries of artworks in the Art knowledge graph.
...
It will return all persons which, according to the facts in the Art knowledge graph, have directly been influenced by Paul Cézanne: 4 artists, including Pablo Picasso, and Vincent van Gogh.
...
*Reasoning* is a major AI technique to derive new knowledge from existing facts in a knowledge graph.
...
Two approaches to rule-based reasoning can be distinguished: *forward chaining* and *backward chaining*. In forward chaining, the reasoning engine starts with the facts and applies all specified rules in order to find all possible conclusions. Queries on this extended knowledge graph will include the logic specified in the rules.
...
[SPARQL Update](https://www.w3.org/TR/sparql11-update) provides means for updating knowledge graphs based on existing facts. This mechanism may be used for forward chaining rule-based reasoning. Since SPARQL update statements are an intuitive extension of SPARQL queries and standardized by the W3C, I will present them in the following section. 

### SPARQL Update

Assume you want to query the Art knowledge graph for artists who are painters as well as sculptors. However, in the Art knowledge graph schema there is no direct representation of the concepts painter and sculptor. 
But you may infer this information from the Art knowledge graph with the following rules:
...
- A *Knowledge Editor* allows editing knowledge graphs. Knowledge graphs may be imported / exported between knowledge editor (development time) and knowledge base (run-time). Standard formats like RDF may be used for importing / exporting.
- *Knowledge resources* include off-the-shelf knowledge graphs like, e.g., Wikidata, that may be used in an AI application.
- *Data Integration / Semantic Enrichment* are services that allow integrating various knowledge resources or subsets thereof. For example, the Art knowledge graph described above is a custom subset of Wikidata.
...
### Knowledge Graph - Make or Buy?

Make or buy? This is common a question in the technology selection phase of an IT project. It also applies to the knowledge graph in an AI application project.

As outlined in the section on Linked Data, there are thousands of knowledge graphs available, many with a public license, with thousands or even hundreds of thousands of facts each. An area with particularly well-established public knowledge graphs are life sciences; See, e.g., [The OBO Foundry](http://www.obofoundry.org/) with knowledge graphs for symptoms, diseases, medications, clinical procedures, genes / proteins, etc. etc. 
There are even *knowledge graph search engines*, e.g., [BARTOC](http://bartoc.org) .

Surprisingly however, when being confronted with concrete requirements for an AI application, it turns out that rarely an off-the-shelf knowledge graph is sufficient. In none of my previous projects in various areas (medicine, manufacturing industry, tourism, libraries, creative arts, and software engineering) I could simply take and use an off-the-shelf knowledge graph without adaptation. Other practitioners have made similar experiences (Ege et al., 2015), (Hoppe et al. 2018).
...
Also, the quality of some public knowledge graphs is mediocre. For example, in Wikidata the class wife is in a transitive `rdfs:subClassOf` relationship with class animal (accessed 2019-04-03). 
...
When implementing the scripts for extracting the Art knowledge graph from Wikidata, we put quite some effort in data cleansing measures, e.g., eliminating birth and death dates that are not valid, birth and death places that are no locations, artist's movements that are no artistic movements etc.

Wikidata is still a suitable source for the Art knowledge graph example in this book and for web apps like [openArtBrowser](https://openartbrowser.org). However, in a project for one of the leading German Arts museums ([Staedel digital collection](https://sammlung.staedelmuseum.de)), the use of general-purpose knowledge graphs like Wikidata was out of question due to their quality deficiencies. 

Of course, it is generally advisable to favor an off-the-shelf knowledge graph over a custom-made one. 
Advantages are:

- Less costs for creating and maintaining the knowledge graph
...
2. Transforming knowledge graph schemas, e.g., from `wd:Q3305213` to `:artwork`
3. Quality enhancement, e.g., omitting birth dates that are not valid
4. Integrating multiple knowledge graphs, e.g., removing duplicates
...
One final remark regarding selecting off-the-shelf knowledge graphs: do not worry too much about technical formats. Converters between XML, CSV, XLS, RDF, database dumps, etc. are available open source or can easily be developed.  
...
11. What is linked data? Give examples of off-the-shelf knowledge graphs.
...
18. How to select knowledge graphs for a concrete application use cases?
