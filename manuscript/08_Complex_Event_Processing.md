...
4. Making purchase recommendations based on click stream analysis in web shops
...
A sliding window can also be used to detect anomalies using machine learning. An anomaly is some unexpected behaviour in data points. I have introduced autoencoder neural networks in Chapter 6. One of their use cases is anomaly detection. The advantage this approach is that it is unsupervised, i.e. costly labelling of (fault) data is not necessary. The consequence is that detected anomalies must be cross-checked by domain experts whether, in fact, they are errors or not.

{width=65%}
![Fig. 8.9: Autoencoder NN for anomaly detection](images/AD_AE.png)

An autoencoder neural network is symmetric with the input layer identical to the output layer (see Fig. 8.9). The hidden layers implement an encoder (compressing the input data) and a decoder (decompressing the compressed data representation). It is trained to minimize the reconstruction error. 
When used for anomaly detection, the autoencoder is trained with normal data, i.e., data when no error is assumed.
In production, live data is fed to the autoencoder. Then, the reconstruction is compared with the input data. If the reconstruction error (e.g., MAE) exceeds a certain pre-defined threshold, then an anomaly is assumed. This is because the network behaves differently from the trained state in which no errors were present.
...
Fig. 8.10 shows the CEP services map. 

{width=75%}
![Fig. 8.10: CEP Services Map](images/CEP_SM.png)
...
Fig. 8.11 shows the CEP product map. 

{width=75%}
![Fig. 8.11: CEP Product Map](images/CEP_PM.png)
